...
try:
    ...
    HAS_ORTOOLS = False
    print("Warning: OR-Tools not available, falling back to bounded BFS")
...
def solve_machine_ilp(buttons: List[List[int]], targets: List[int]) -> int:
    ...
    if not solver:
        ...
        if not solver:
            print("Warning: No ILP solver available, using bounded BFS")
            return solve_machine_bounded_bfs(buttons, targets)
...
def solve_machine_bounded_bfs(buttons: List[List[int]], targets: List[int], max_presses_per_button: int = 300) -> int:
    """Solve using bounded BFS (fallback method)."""
    n = len(targets)  # number of counters
    m = len(buttons)  # number of buttons
...
def solve_machine_part2_linear(buttons: List[List[int]], targets: List[int]) -> int:
    """Solve for minimum button presses for Part 2 using ILP (or BFS fallback)."""
...
def main():
    ...
    for line in lines:
        ...
        if min_presses == -1:
            print(f"Machine {len(targets)} counters, {len(buttons)} buttons: No solution found")
            continue
